...
@Component({
  ...
  template: `
    <div class="card">
      ...
      <p style="margin-top:8px; color:#6b7280;">Aprueba o rechaza eventos pendientes.</p>

      <div
        *ngIf="mensaje"
        class="card"
        style="margin-top:12px; border-left:4px solid;"
        [ngStyle]="{ 'border-left-color': tipoMensaje === 'ok' ? '#16a34a' : '#dc2626' }"
      >
        <div [style.color]="tipoMensaje === 'ok' ? '#166534' : '#991b1b'" style="font-weight:600;">
          {{ mensaje }}
        </div>
      </div>
        ...
        <div style="display:flex; justify-content:space-between; gap:10px;">
          <div>
            ...
            <div style="margin-top:10px; display:flex; gap:10px; flex-wrap:wrap;">
              <a
                *ngIf="e.rutaInformePdf"
                class="btn btn-naranja"
                [href]="apiUrl + e.rutaInformePdf"
                target="_blank"
              >
...
>
  {{e.estado}}
        ...
        <div style="margin-top:10px;">
          ...
          <input
            class="input"
            [(ngModel)]="observaciones[e.id]"
            placeholder="Obligatorio si rechazas"
          />
        </div>

        <div style="margin-top:10px; display:flex; gap:10px;">
          <button type="button" class="btn btn-verde" (click)="decidir(e.id, 'APROBADO')">
            <span class="material-icons">check_circle</span>
            Aprobar
          </button>
          <button type="button" class="btn btn-rojo" (click)="decidir(e.id, 'RECHAZADO')">
      ...
      next: () => {
        ...
        this.mensaje =
          estado === 'APROBADO'
            ...
            : 'Evento rechazado y observación enviada.';
        this.cargar();
      },
      error: (err: any) => {
        this.tipoMensaje = 'error';
