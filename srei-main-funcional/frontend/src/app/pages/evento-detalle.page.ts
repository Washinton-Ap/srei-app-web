    ...
    <div class="card">
      <div style="display:flex; justify-content:space-between; gap:12px; align-items:flex-start;">
        <div>
          ...
          <div style="margin-top:6px; color:#6b7280;">{{evento.fecha}} · {{evento.lugar}} · {{evento.ambito}}</div>
          <div style="margin-top:6px; display:flex; flex-wrap:wrap; gap:8px; align-items:center;">
            ...
            <span class="pill" *ngIf="evento.docenteCorreo">Docente: {{evento.docenteCorreo}}</span>
          </div>
          <div *ngIf="evento.estado === 'RECHAZADO' && evento.observaciones" class="card" style="margin-top:10px; border-left:4px solid #b91c1c;">
      ...
      <div *ngIf="qr" style="margin-top:14px; display:flex; gap:14px; align-items:flex-start; flex-wrap:wrap;">
        <img [src]="'data:image/png;base64,' + qr.pngBase64" width="160" height="160" style="border:1px solid var(--borde); border-radius:12px; padding:8px; background:white;" />
        <div style="min-width:260px;">
          ...
          <div *ngIf="mensajeAsistencia" class="card" style="margin-top:10px; border-left:4px solid;"
               [ngStyle]="{ 'border-left-color': tipoMensajeAsistencia === 'ok' ? '#16a34a' : '#dc2626' }">
            <div [style.color]="tipoMensajeAsistencia === 'ok' ? '#166534' : '#991b1b'" style="font-weight:600;">
              {{mensajeAsistencia}}
            </div>
          </div>
        </div>
    ...
    <div class="layout" style="margin-top:14px;">
      <div class="sidebar">
        ...
        <div class="card" style="margin-top:12px;" *ngIf="ranking.length">
          ...
          <div *ngFor="let r of ranking; let i = index" style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid var(--borde);">
      ...
      <div>
        <div class="card">
          ...
          <div *ngFor="let c of comentarios" style="border-bottom:1px solid var(--borde); padding:10px 0;">
        ...
        <div class="card" style="margin-top:12px;" *ngIf="preguntas.length">
          ...
          <div *ngFor="let p of preguntas" style="border:1px solid var(--borde); border-radius:12px; padding:12px; margin-bottom:10px;">
            <div style="font-weight:600;">{{p.enunciado}}</div>
            <div style="margin-top:8px; display:grid; gap:8px;">
              <button class="btn" *ngFor="let op of p.opciones; let idx = index" (click)="responder(p.id, idx)">
...
export class EventoDetallePage {
  ...
  private cargarDetalle(id: number) {
    ...
    this.cargarRanking();
      ...
      error: (err) => {
        ...
        this.errorCarga = msg ? String(msg) : `No se pudo cargar el evento (HTTP ${err?.status || '???'}).`;
      }
    });
  }
  ...
  confirmarAsistencia() {
    ...
    this.asistenciaService.confirmarEvento(this.evento.id).subscribe({
      ...
      error: (e) => {
        ...
        this.mensajeAsistencia = msg ? String(msg) : `No se pudo confirmar la asistencia (HTTP ${e?.status || '???'}).`;
      }
    });
  }
  ...
  cargarTrivia() {
    if (!this.evento) return;
    this.triviaService.preguntas(this.evento.id).subscribe({ next: (p) => (this.preguntas = p || []) });
  }
  ...
  abrirInforme() {
    ...
    this.eventoService.informeBlob(this.evento.id).subscribe({
      ...
      error: (e) => {
        this.tipoMensajeAsistencia = 'error';
        const msg = e?.error?.message || e?.error?.error || e?.message;
        this.mensajeAsistencia = msg ? String(msg) : `No se pudo abrir el informe (HTTP ${e?.status || '???'}).`;
      }
    });
  }
